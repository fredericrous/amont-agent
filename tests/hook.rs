...
/// has read and believed it. A stale block is named; a current one, and a
/// repository without one, say nothing.
#[test]
fn a_session_opening_on_a_stale_guidance_block_is_told() {
    let (_, work) = a_stale_clone(); // `work` is up to date with origin
    let stale = format!(
        "# Project\n\n{}\nSTALE\n{}\n",
        amont_runtime::agents_md::START,
        amont_runtime::agents_md::END
    );
    std::fs::write(work.join("AGENTS.md"), stale).unwrap();
    let r = send(&session_start(&work));
    let doc = r.json().expect("a decision document");
    let text = doc["hookSpecificOutput"]["additionalContext"]
        .as_str()
    ...
    // Regenerated by the runtime itself: current, so silence.
    amont_runtime::agents_md::write(&work.join("AGENTS.md")).unwrap();
    let r = send(&session_start(&work));
